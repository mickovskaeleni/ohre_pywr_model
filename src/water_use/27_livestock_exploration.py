import pandas as pd
import pyreadr

from pathlib import Path

# ======================================================
# PATHS
# ======================================================

BASE_DIR = Path(__file__).resolve().parents[2]

DATA_DIR = BASE_DIR / "data"
PROCESSED_DIR = DATA_DIR / "processed"
OUTPUT_DIR = BASE_DIR / "outputs"

DESC_PATH = (
    PROCESSED_DIR /
    "water_use_data_descriptive_treated_w_2009.rds"
)

PILOT_TS_PATH = (
    PROCESSED_DIR /
    "pilot_abstractions_timeseries.csv"
)

# ======================================================
# LOAD DATA
# ======================================================

print("Loading descriptive data...")

desc = pyreadr.read_r(
    DESC_PATH
)[None]

print("Loading pilot abstractions...")

pilot_ts = pd.read_csv(
    PILOT_TS_PATH
)

# ======================================================
# PILOT IDS
# ======================================================

pilot_ids = pilot_ts["ID"].unique()

pilot_desc = desc[
    desc["ID"].isin(pilot_ids)
].copy()

print(
    f"\nPilot abstractions: {len(pilot_desc):,}"
)

# ======================================================
# DIAGNOSTIC 1
# WHAT IS OD_ZIV_V?
# ======================================================

print("\n====================")
print("OD_ZIV_V DIAGNOSTIC")
print("====================")

print("\nDescribe:")

print(
    pilot_desc["OD_ZIV_V"]
    .describe()
)

print("\nValue counts:")

print(
    pilot_desc["OD_ZIV_V"]
    .value_counts(
        dropna=False
    )
    .head(20)
)

print("\nSample rows:")

print(
    pilot_desc[
        [
            "ID",
            "NAZICO",
            "OD_ZIV_V"
        ]
    ]
    .head(30)
)

# ======================================================
# OTHER INDUSTRY FILTER
# ======================================================

livestock = pilot_desc[
    pilot_desc["OD_ZIV_V"].fillna(0) > 0
].copy()

print("\n====================")
print("FILTER RESULTS")
print("====================")

print(
    f"OD_ZIV_V abstractions: {len(livestock):,}"
)

print(
    f"Unique IDs: {livestock['ID'].nunique():,}"
)

if len(livestock) == 0:
    print(
        "\nNo OD_ZIV_V abstractions found."
    )
    raise SystemExit()

# ======================================================
# JOIN TO MVM TIMESERIES
# ======================================================

merged = pilot_ts.merge(
    livestock[
        [
            "ID",
            "NAZICO",
            "OBEC",
            "OKRES",
            "KRAJ"
        ]
    ],
    on="ID",
    how="inner"
)

# ======================================================
# CLEAN NAMES
# ======================================================

merged["OBEC"] = (
    merged["OBEC"]
    .astype(str)
    .str.strip()
    .str.title()
)

merged["NAZICO"] = (
    merged["NAZICO"]
    .astype(str)
    .str.strip()
)

# Optional:
# merge multiple internal spaces into one

merged["NAZICO"] = (
    merged["NAZICO"]
    .str.replace(r"\s+", " ", regex=True)
)

print(
    f"\nRows after join: {len(merged):,}"
)

# ======================================================
# YEAR
# ======================================================

merged["DTM"] = pd.to_datetime(
    merged["DTM"]
)

merged["YEAR"] = (
    merged["DTM"]
    .dt.year
)

# ======================================================
# YEARLY TREND
# ======================================================

yearly = (
    merged
    .groupby("YEAR")["MVM"]
    .sum()
    .reset_index()
    .sort_values("YEAR")
)

# ======================================================
# TOP USERS
# ======================================================

top_users = (
    merged
    .groupby(
        [
            "ID",
            "NAZICO",
            "OBEC"
        ]
    )["MVM"]
    .sum()
    .reset_index()
    .sort_values(
        "MVM",
        ascending=False
    )
)

# ======================================================
# TOP MUNICIPALITIES
# ======================================================

top_municipalities = (
    merged
    .groupby("OBEC")["MVM"]
    .sum()
    .reset_index()
    .sort_values(
        "MVM",
        ascending=False
    )
)

# ======================================================
# CONCENTRATION
# ======================================================

user_totals = (
    top_users["MVM"]
    .sort_values(
        ascending=False
    )
    .reset_index(drop=True)
)

total_volume = user_totals.sum()

top5_share = (
    user_totals.head(5).sum()
    / total_volume
    * 100
)

top10_share = (
    user_totals.head(10).sum()
    / total_volume
    * 100
)

# ======================================================
# SUMMARY
# ======================================================

print("\n====================")
print("OD_ZIV_V SUMMARY")
print("====================")

print(
    f"Total volume: {total_volume:,.0f}"
)

print(
    f"Top 5 share: {top5_share:.1f}%"
)

print(
    f"Top 10 share: {top10_share:.1f}%"
)

print("\n====================")
print("TOP 20 USERS")
print("====================")

print(
    top_users.head(20)
)

print("\n====================")
print("TOP 20 MUNICIPALITIES")
print("====================")

print(
    top_municipalities.head(20)
)

print("\n====================")
print("YEARLY TREND")
print("====================")

print(yearly.head())

print(yearly.tail())

# ======================================================
# SAVE
# ======================================================

top_users.to_csv(
    OUTPUT_DIR /
    "livestock_top_users.csv",
    index=False
)

top_municipalities.to_csv(
    OUTPUT_DIR /
    "livestock_top_municipalities.csv",
    index=False
)

yearly.to_csv(
    OUTPUT_DIR /
    "livestock_yearly_trend.csv",
    index=False
)

print("\nSaved:")
print(
    OUTPUT_DIR /
    "livestock_top_users.csv"
)
print(
    OUTPUT_DIR /
    "livestock_top_municipalities.csv"
)
print(
    OUTPUT_DIR /
    "livestock_yearly_trend.csv"
)

print("\nDone.")