...
irrigation = pilot_desc[
    pilot_desc["OD_ZAVL"].fillna(0) > 0
].copy()
...
print(
    f"OD_ZAVL abstractions: {len(irrigation):,}"
)

print(
    f"Unique IDs: {irrigation['ID'].nunique():,}"
)

if len(irrigation) == 0:
    print(
        "\nNo OD_ZAVL abstractions found."
    )
    raise SystemExit()
...
# Optional:
# merge multiple internal spaces into one
...
top_users.to_csv(
    OUTPUT_DIR /
    "irrigation_top_users.csv",
    index=False
)

top_municipalities.to_csv(
    OUTPUT_DIR /
    "irrigation_top_municipalities.csv",
    index=False
)

yearly.to_csv(
    OUTPUT_DIR /
    "irrigation_yearly_trend.csv",
    index=False
)
